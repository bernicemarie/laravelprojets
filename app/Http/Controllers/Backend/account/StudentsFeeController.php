...
class StudentsFeeController extends Controller
{
  ...
  $data = StudentRegistration::with(['registration_relation_discount'])->where('year_id',$year_id)->where('class_id',$class_id)->get();
...
if($accountstudentfees !=null) {
    ...
    $color = 'success';
    $html[$key]['tdsource']  = '<td>'.$std['registration_relation_user']['id_no']. '<input type="hidden" name="fee_category_id" value= " '.$fee_category_id.' " >'.'</td>';

    $html[$key]['tdsource']  .= '<td>'.$std['registration_relation_user']['name']. '<input type="hidden" name="year_id" value= " '.$std->year_id.' " >'.'</td>';

    $html[$key]['tdsource']  .= '<td>'.$std['registration_relation_user']['fname']. '<input type="hidden" name="class_id" value= " '.$std->class_id.' " >'.'</td>';

    $html[$key]['tdsource']  .= '<td>'.$registrationfee->amount.'$'.'<input type="hidden" name="date" value= " '.$date.' " >'.'</td>';

    $html[$key]['tdsource'] .= '<td>'.$std['registration_relation_discount']['discount'].'%'.'</td>';
