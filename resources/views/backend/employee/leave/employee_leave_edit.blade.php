		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			  <h4 class="box-title">Editer Départ Employé</h4>
...
<div class="col-md-6">
	...
	<div class="controls">
	 <select name="employee_id" required="" class="form-control">
			<option value="" selected="" disabled="">Selectionner un employé</option>
 ...
 <option value="{{ $employee->id }}" {{ ($editData->employee_id == $employee->id)? 'selected':'' }}  >{{ $employee->name }}</option>
   ...
   <div class="form-group">
	<h5>Départ <span class="text-danger">*</span></h5>
	<div class="controls">
	 <select name="leave_purpose_id" id="leave_purpose_id" required="" class="form-control">
			<option value="" selected="" disabled="">Selectionner un sujet </option>
 ...
 <option value="{{ $leave->id }}" {{ ($editData->leave_purpose_id == $leave->id)?'selected':'' }}  >{{ $leave->name }}</option>
		...
		<br>
<input type="text" name="name" id="add_another" class="form-control" placeholder="Ajouter un sujet" style="display: none;">		
	 </div>
          </div>
	 ...
	 <input type="reset" class="btn btn-rounded btn-info mb-5" value="Annuler">
						</div>
					</form>
